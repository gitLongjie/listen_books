...
class AudioPlayerPage extends StatefulWidget {
  // 将默认构造函数私有化
  const AudioPlayerPage._();

  // 外部跳转统一经过这儿
  static void gotoPlayer(BuildContext context, {required List list, int? index}) {
    if (list != null) {
      Provider.of<MusicController>(context, listen: false).setPlayList(list, index!);
    }
    NavigatorUtil.push(context, const AudioPlayerPage._());
  }
...
class _AudioPlayerPageState extends State<AudioPlayerPage>
  ...
  late MusicListener musicListener;
  ...
  Widget _buildControllerBar() {
    // 循环方式
    CycleType cycleType = musicController.playList._cycleType;
    return Container(
